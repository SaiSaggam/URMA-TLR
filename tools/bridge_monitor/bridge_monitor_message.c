 ...
 * new thread.
 */
STATIC_FUNCTION int bridgemon_message_callback(int fd, size_t const instance, message_codes_t const message_code, void * * const info, size_t const info_size)
{
    (void)fd;
    return send_event_to_bridge_monitor(instance, message_code, info, info_size);
}

/*
 * Start listening for messages from other processes.
*/
bool bridge_messaging_setup(void)
{
    bool result;

    message_context = open_messaging_context(bridge_monitor, bridgemon_message_callback);
    if (message_context == NULL)
    {
        bridgemon_error("could not open messaging context\n");
        result = false;
        goto done;
    }
...
*/
void bridge_messaging_teardown(void)
{
