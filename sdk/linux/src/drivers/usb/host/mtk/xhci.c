...
static unsigned int end_point_cnt = 0;
module_param(end_point_cnt, uint, S_IRUGO);
MODULE_PARM_DESC(end_point_cnt, "Counter of the used end point");

static int ep_restrict =1;
module_param(ep_restrict, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(ep_restrict, "To restrict used ep");
...
//check the ep of specified device can be used or not
struct allow_ep_list
{
	unsigned int vid;
	unsigned int pid;
	unsigned char ep[32];
...
static const struct allow_ep_list allow_ep[] = 
{
	//the last one data in ep array should be 0x00 for ending
	{0x1435,0xd181,{0x03,0x04,0x05,0x06,0x83,0x84,0x85,0x86,0x87,0x88,0x89,0x8a,0x00}}, //WNC D18QX
	{0x2001,0x7d04,{0x01,0x02,0x03,0x81,0x82,0x83,0x87,0x88,0x00}},	//D-Link P300A
	{0,0,0}
};


int check_ep_allowable(struct usb_device *udev,struct usb_host_endpoint *ep)
{
	unsigned int ind=0;
	while (allow_ep[ind].ep[0] !=0)
	{
		if ((allow_ep[ind].vid == udev->descriptor.idVendor) && 
		   (allow_ep[ind].pid == udev->descriptor.idProduct))
		{
			unsigned int ind2 = 0;
			while (allow_ep[ind].ep[ind2] != 0x00)
			{
				//printk("Check ep [0x%02X]\n",allow_ep[ind].ep[ind2]);
				if (allow_ep[ind].ep[ind2] == ep->desc.bEndpointAddress)
					return 0;
				ind2++;
			}
			return 1;
		...
		struct usb_host_endpoint *ep)
{
	...
	if (ep_restrict)
	{
		if (check_ep_allowable(udev,ep))
		{
			printk("%s:Ignore endpoint of device with DeviceID[0x%04X/0x%04X] and endpoint [0x%02X]\n",
				__FUNCTION__,udev->descriptor.idVendor,
				udev->descriptor.idProduct,ep->desc.bEndpointAddress);
            		return 0;
		}

	}
#if 0
        if(ep->desc.bEndpointAddress == 0x8b ||  ep->desc.bEndpointAddress == 0x07 ||
		   ep->desc.bEndpointAddress == 0x82 ||  ep->desc.bEndpointAddress == 0x02 )
        {
            printk(" %s: ignor endpoint address 0x%x \n", __FUNCTION__, ep->desc.bEndpointAddress);
            return 0;
        }
#endif
	...
	if (end_point_cnt > 0) end_point_cnt --;
		...
		struct usb_host_endpoint *ep)
{
	...
	if (added_ctxs == SLOT_FLAG || added_ctxs == EP0_FLAG) {
		...
		//xhci_dbg(xhci, "xHCI %s - can't add slot or ep 0 %#x\n",
		  printk( "xHCI %s - can't add slot or ep 0 %#x\n",
				__func__, added_ctxs);
		return 0;
	}
	...
	//printk("Add endpoint for vendor [0x%04X] product [0x%04X] end point [0x%02X]\n",
	...
	if (virt_dev->eps[ep_index].ring &&
		...
		printk("Trying to add endpoint 0x%x ",
				(unsigned int) ep->desc.bEndpointAddress);
		return -EINVAL;
	}
	...
	if (le32_to_cpu(ctrl_ctx->add_flags) &
		...
		printk("xHCI %s called with enabled ep %p\n",
				__func__, ep);
		return 0;
	}
	...
	if (xhci_endpoint_init(xhci, virt_dev, udev, ep, GFP_NOIO) < 0) {
		...
		printk("%s - could not initialize ep %#x\n",
	...
	in_ep_ctx = xhci_get_ep_ctx(xhci, in_ctx, ep_index);
		...
		if(mtk_xhci_scheduler_add_ep(udev->speed, usb_endpoint_dir_in(&ep->desc),
			...
			printk("[MTK] not enough bandwidth\n");
			return -ENOSPC;
		}
#else
...
int xhci_gen_setup(struct usb_hcd *hcd, xhci_get_quirks_t get_quirks)
{
	...
	//hcd->self.no_sg_constraint = 1; //by tt
