			 ...
			 const char *buf, size_t count)
{
	...
	unsigned int bInterfaceNumber = 0;
	int fields = 0;
	int retval = 0;

	fields = sscanf(buf, "%x %x %x %x", &idVendor, &idProduct, &bInterfaceClass, &bInterfaceNumber);
	...
	if (fields == 3) {
	   ...
	   dynid->id.bInterfaceClass = (u8)bInterfaceClass;
	   dynid->id.match_flags |= USB_DEVICE_ID_MATCH_INT_CLASS;
	   dynid->id.bInterfaceNumber = (u8)bInterfaceNumber;
	   dynid->id.match_flags |= USB_DEVICE_ID_MATCH_INT_NUMBER;
	}
