...
static int wdt_timeout = 0;     /* timeout from user space */ 
static u32 WdgLoadValue1;
...
static void RaWdgStart(void)
{
    ...
    WdgLoadValue = wdt_timeout * 1000;
    sysRegWrite(TMR1LOAD,  WdgLoadValue);
    printk(KERN_INFO "Started WatchDog Timer. LoadVlaue1: 0x%08x TIMEOUT: %d\n",WdgLoadValue, wdt_timeout);
#else
    SetWdgTimerClock(TMR1CTL,SYS_CLK_DIV65536);
		...
		case WDIOC_SETTIMEOUT: 
		{
			if (get_user(options, (int *)arg)) 
			{
				return -EFAULT;
      			...
      			if(options > 9) 
      			{                                
	      			printk(KERN_INFO "Set Watchdog Timtout to %d\n", options);
	      			wdt_timeout = options;
	      			RaWdgStart();
      			}
      			else if(options == 0)
        		...
        		if(kernel_mode) 
        		{	
          			printk(KERN_INFO "Enable Kernel Mode Watchdog\n");
          			kernel_mode = 0;
        		}
        		else 
        		{
          			printk(KERN_INFO "Disable Kernel Mode Watchdog\n");
          			kernel_mode = 1;
        		}                
				RaWdgStart();
      			}
      			...
      			{
      				printk(KERN_INFO "Watchdog Timtout should not less than 10 \n");
      			}
      			return 0;
    		}
