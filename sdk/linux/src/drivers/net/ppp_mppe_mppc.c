...
{
    crypto_blkcipher_setkey(state->arc4, key, keylen);
}
	     ...
	     const unsigned int len, unsigned char *out)
{
    ...
    crypto_blkcipher_encrypt(&desc, sgout, sgin, len);
}
...
get_new_key_from_sha(struct ppp_mppe_state *state, unsigned char *interim_key)
{
	...
	nbytes += setup_sg(&sg[1], sha_pad->sha_pad1, sizeof(sha_pad->sha_pad1));
	nbytes += setup_sg(&sg[2], state->session_key, state->keylen );
	nbytes += setup_sg(&sg[3], sha_pad->sha_pad2, sizeof(sha_pad->sha_pad2));
...
static int __init mppe_module_init(void)
{
        int answer;
