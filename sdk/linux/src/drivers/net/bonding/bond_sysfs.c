...
/*
 * Show and set the sales auto_weight flag.
 */
static ssize_t bonding_show_auto_weight(struct device *d,
				    struct device_attribute *attr,
				    char *buf)
{
	struct bonding *bond = to_bond(d);

	return sprintf(buf, "%d\n", bond->params.auto_weight);
}

static ssize_t bonding_store_auto_weight(struct device *d,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	int new_value, ret = count;
	...
	if (sscanf(buf, "%d", &new_value) != 1) {
		pr_err("%s: no auto_weight value specified.\n",
		       bond->dev->name);
		ret = -EINVAL;
		goto out;
	}
	if ((new_value == 0) || (new_value == 1)) {
		bond->params.auto_weight = new_value;
		pr_info("%s: Setting auto_weight to %d.\n",
			bond->dev->name, new_value);
	} else {
		pr_info("%s: Ignoring invalid auto_weight value %d.\n",
			bond->dev->name, new_value);
	}
out:
	return count;
...
static DEVICE_ATTR(auto_weight, S_IRUGO | S_IWUSR,
		   bonding_show_auto_weight, bonding_store_auto_weight);
