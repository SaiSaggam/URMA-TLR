...
static unsigned int queue_mem = 60;
module_param(queue_mem, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC (queue_mem, "Size of Queue");
#define	MAX_QUEUE_MEMORY	(queue_mem * 1518)
#define	RX_QLEN(dev)		((dev)->rx_qlen)
#define	TX_QLEN(dev)		((dev)->tx_qlen)

static unsigned int rx_queue = 10;
module_param(rx_queue, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC (rx_queue, "Size of RX Queue");
...
static unsigned int max_rx_urb = 6144;
module_param(max_rx_urb, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC (max_rx_urb, "Max. RX URB");
//#define MAX_RX_URB(x)   (x > max_rx_urb ? x : max_rx_urb)
#define MAX_RX_URB(x)   ((x > max_rx_urb) ? max_rx_urb : x)
/*-------------------------------------------------------------------------*/
...
void usbnet_update_max_qlen(struct usbnet *dev)
{
    ...
    printk("Queue of usbnet RX [%d][%d] TX[%d][%d]\n",
        dev->rx_urb_size,dev->rx_qlen,dev->hard_mtu,dev->tx_qlen);
}
...
static int rx_submit (struct usbnet *dev, struct urb *urb, gfp_t flags)
{
	...
	skb = __netdev_alloc_skb(dev->net, size+NET_IP_ALIGN+FOE_INFO_LEN, flags);
#else
...
static int rx_alloc_submit(struct usbnet *dev, gfp_t flags)
{
	...
	for (i = 0; i < rx_queue && dev->rxq.qlen < RX_QLEN(dev); i++) {
...
// dma_supported() is deeply broken on almost all architectures
	...
	if (!dev->rx_urb_size)
		dev->rx_urb_size = MAX_RX_URB(dev->hard_mtu);
	dev->maxpacket = usb_maxpacket (dev->udev, dev->out, 1);

	/* let userspace know we have a random address */
	//if (ether_addr_equal(net->dev_addr, node_id))
	if (compare_ether_addr(net->dev_addr, node_id))
