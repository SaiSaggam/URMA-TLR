...
#endif
/////////////////////////////////////////////////////////////////////////////////////////////////
#define DRIVER_VERSION "v2.10.00.00"
...
#define MAX_RX_URB_BUF	16384

//#define MAX_RX_URB(x)	(x > MAX_RX_URB_BUF) ? MAX_RX_URB_BUF : (x) 
static unsigned int max_rx_urb_buf = 6144;
//module_param(rt_debug, bool, S_IRUGO|S_IWUSR);
module_param(max_rx_urb_buf, uint, S_IRUGO | S_IWUSR);
#define MAX_RX_URB(x)	(x > max_rx_urb_buf) ? max_rx_urb_buf : (x) 
...
#define HW_USB_MRECEIVE_MAX_BUFFER_SIZE (1024 * 6)  
...
#define NCM_TX_DEFAULT_TIMEOUT_MS 4
...
static unsigned short ncm_read_size_in1k = 32;
...
//#define NCM_NTB_HARD_MAX_IN_SIZE ((u32)(6 * 1024))
#else
...
static int hw_change_mtu (struct net_device *net, int new_mtu)
{
    ...
    if (dev->rx_urb_size == old_hard_mtu && !dev->is_ncm) {
		...
		printk("Lar [%d] rx_urb_buf = [%d]\n",__LINE__,dev->rx_urb_size);
...
/* Add for DTS2011050903736 lxz 20110520 end*/
    ...
    if (dev->is_ncm){
           dev->rx_urb_size = MAX_RX_URB(dev->ncm_ctx->rx_max_ntb);
		printk("Lar [%d] rx_urb_buf = [%d]\n",__LINE__,dev->rx_urb_size);
...
next_desc:
    ...
    if(dev->is_ncm)
    {
        ...
        dev->rx_urb_size = MAX_RX_URB(ctx->rx_max_ntb);
		printk("Lar [%d] rx_urb_buf = [%d]\n",__LINE__,dev->rx_urb_size);
