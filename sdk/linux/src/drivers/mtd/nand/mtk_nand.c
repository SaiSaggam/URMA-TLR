...
static int get_start_end_block(struct mtd_info *mtd, int block, int *start_blk, int *end_blk)
{
	...
	*start_blk = 0;
		...
		else if ((g_pasStatic_Partition[i].size == MTDPART_SIZ_FULL) || (g_pasStatic_Partition[i].size == FLASHMAP_WHOLE_SIZE))
		{
			continue;
		}
...
static int block_remap(struct mtd_info *mtd, int block)
{
	...
	//printk("\n 1 get_start_end_block %x %x %x %x \n",block_offset,start_blk,end_blk,block);
	...
	if (j <= end_blk)
	{
	//	printk("\n 2 get_start_end_block bad_block %x %x \n",bad_block,j);
		return j;
	}
	else
	{
		
		//printk("3 get_start_end_block %x %x\n",j,bad_block);
		...
		for (j = end_blk; bad_block > 0; j--)
		{
			...
			{
				bad_block--;
			//	printk("3 get_start_end_block %x %x %x\n",j,bad_block,block_offset);			
...
static int is_skip_bad_block(struct mtd_info *mtd, int page)
{
	...
	count ++;
#ifdef NAND_JFFS2_WORKAROUND // NOTE: here maybe need to customize ; by aaron 2015/8/14 ¤U¤È 04:25:55
if (((page << chip->page_shift) >= FLASHMAP_STORAGE_OFFSET) && ((page << chip->page_shift) < (FLASHMAP_STORAGE_OFFSET+FLASHMAP_STORAGE_SIZE)))
#else
...
static void mtk_nand_select_chip(struct mtd_info *mtd, int chip)
{
	...
	MSG(INIT, "===>*** mtd->writesize=%d mtd->oobsize=%d,	mtd->erasesize=%d , spare_per_sector=%d\n",mtd->writesize,mtd->oobsize, mtd->erasesize,spare_per_sector);
;
...
#endif
{
	...
	MSG(INIT, "===>*** mtd->writesize=%d mtd->oobsize=%d,	mtd->erasesize=%d  devinfo.iowidth=%d ,devinfo.sparesize==%d \n",mtd->writesize,mtd->oobsize, mtd->erasesize,devinfo.iowidth,devinfo.sparesize);
...
//#endif		
//#endif
	err = add_mtd_partitions(mtd, g_pasStatic_Partition, ARRAY_SIZE(g_pasStatic_Partition));
