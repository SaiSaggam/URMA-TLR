...
#ifndef USE_KERNEL
#if 1
        /* Wait for data to arrive from pppd before proceeding */
        {
	  fd_set pty_fds;
	  struct timeval idleTime;
	  int maxfd = m_pty;
          extern int errno;
          l2tp_db(-1, "Waiting for first packet from pppd\n");
	  FD_ZERO(&pty_fds);
	  FD_SET(m_pty, &pty_fds);
	  idleTime.tv_sec = 10;
	  idleTime.tv_usec = 0;
          while(1) {
	    switch (select(maxfd + 1, &pty_fds, NULL, NULL, &idleTime)) {
	      case -1:
                if(errno == EINTR) {
                  l2tp_db(-1, "Got EINTR, Retry Select\n");
                  continue;
                }
                l2tp_db(-1, "Select on pty failed, ignoring: %s\n",
                        strerror(errno));
	        break;
	      case 0:
                l2tp_db(-1, "Timeout waiting for first packet from our pppd\n");
	        break;
              default:
                l2tp_db(-1, "Select took %d.%6.6d seconds\n",
                        9-idleTime.tv_sec, 1000000-idleTime.tv_usec);
                break;
	    }
            break;
	  }
