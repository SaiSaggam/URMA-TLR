...
main(int argc, char *argv[])
{
    ...
    int do_fork = 1;
    int debugmask = 0;
    char *config_file = "/etc/l2tp/l2tp.conf";
    ...
    /* Get reconfig file name */
    static int InitFlag = 1;
    char reconfig_filename[1024] = "";
    snprintf(reconfig_filename, sizeof(reconfig_filename), "%s-reconf", pidfile);

#ifdef USE_KERNEL
    static int kernel_mode = 1;
    /* Check if user mode or kernel mode by checking config file argument "kernel-mode" */
    FILE *configfile_fp = fopen(config_file, "r");
    if (configfile_fp != NULL)
    {
        char ConfigBuf[4096] = "";
        fread(ConfigBuf, sizeof(char), sizeof(ConfigBuf), configfile_fp);
        fclose(configfile_fp);
        if (strstr(ConfigBuf, "kernel-mode 0") != NULL) {kernel_mode = 0;}
        memset(ConfigBuf, 0, sizeof(ConfigBuf));
    }  
#else
    static int kernel_mode = 0;
#endif

    while(1) {

    int ReconfFlag = 0;
    FILE *fp = fopen(reconfig_filename, "r");
    if (fp != NULL)
    {
        fclose(fp);
        remove(reconfig_filename);
        ReconfFlag = 1;
    }
    else if (InitFlag != 0)
    {
    ...
    if(ReconfFlag != 0){
        /* Avoid config parsing failed issue on l2tp user mode */
        if (kernel_mode == 0)
        {
           fprintf(stderr, "l2tp prepare to re-parse configure file on user mode (%s)", reconfig_filename);
           l2tp_peer_init();
           l2tp_parse_config_file(es, config_file);
        }
        else {fprintf(stderr, "l2tp do not re-parse configure file on kernel mode (%s)", reconfig_filename);}
    }
