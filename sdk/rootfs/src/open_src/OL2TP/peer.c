...
static l2tp_opt_descriptor peer_opts[] = {
    ...
    { "rws",               OPT_TYPE_INT,      &prototype.rws}, //20150325 Fish Extend for rws
		    ...
		    char const *value)
{
	...
	*/
	...
	peer->rws = prototype.rws; //20150325 Fish extend for rws
	return 0;
    }
...
l2tp_peer_find(struct sockaddr_in *addr, char const *peername)
{
	...
	if ((candidate->addr.sin_addr.s_addr & mask) ==
		...
		!strcmp(peername,addr1_str)||
		!strcmp(peername,candidate->hostname)||
	...
	if(peername && (candidate->peername[0]) && (!strcmp(peername,candidate->peername)))
    	{
    	peer = candidate;
