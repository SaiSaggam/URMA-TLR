                        ...
                        WriteMethod **write_method )
{
  ...
  for ( np = LIST_FIRST ( &prList ); np != NULL; np = LIST_NEXT ( np, nodes )) {
    ppr = ( struct prTbl_data * ) np->data;
    rName[vp->namelen] = ppr->ifIndex;
    if ((  exact && ( snmp_oid_compare ( rName, vp->namelen + 1, name, *length ) == 0 )) ||
        ( !exact && ( snmp_oid_compare ( rName, vp->namelen + 1, name, *length ) >  0 ))) {
      switch ( vp->magic ) {
        case DOT11PRIVACYINVOKED:
          if ( ppr->havePrivacyInvoked      ) found = TRUE; break;
        case DOT11WEPDEFAULTKEYID:
          if ( ppr->haveWEPDefaultKeyID     ) found = TRUE; break;
        case DOT11WEPKEYMAPPINGLENGTH:
          if ( ppr->haveWEPKeyMappingLength ) found = TRUE; break;
        case DOT11EXCLUDEUNENCRYPTED:
          if ( ppr->haveExcludeUnencrypted  ) found = TRUE; break;
        case DOT11WEPICVERRORCOUNT:
          if ( ppr->haveWEPICVErrorCount    ) found = TRUE; break;
        case DOT11WEPEXCLUDEDCOUNT:
          if ( ppr->haveWEPExcludedCount    ) found = TRUE; break;
      }      
    }
    if ( found )
      break;
...
//    *write_method = write_dot11PrivacyInvoked;
      return ( UCHAR * ) &ppr->privacyInvoked;

    case DOT11WEPDEFAULTKEYID:
//    *write_method = write_dot11WEPDefaultKeyID;
      return ( UCHAR * ) &ppr->WEPDefaultKeyID;

    case DOT11WEPKEYMAPPINGLENGTH:
//    *write_method = write_dot11WEPKeyMappingLength;
      return ( UCHAR * ) &ppr->WEPKeyMappingLength;

    case DOT11EXCLUDEUNENCRYPTED:
//    *write_method = write_dot11ExcludeUnencrypted;
      return ( UCHAR * ) &ppr->excludeUnencrypted;

    case DOT11WEPICVERRORCOUNT:
      return ( UCHAR * ) &ppr->WEPICVErrorCount;
        
    case DOT11WEPEXCLUDEDCOUNT:
      return ( UCHAR * ) &ppr->WEPExcludedCount;
