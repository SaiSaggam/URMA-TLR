...
/* Older kernels / toolchains don't have this function */
...
static int inotify_init1(int flags)
{
	...
	if (flags & IN_CLOEXEC) {
		if ( (fcntl_flags = fcntl(fd, F_GETFD)) < 0 ||
...
{
  struct resolvc *res;
#ifdef DIGI_RESOLV_SYMLINK_EXTENSION
  struct resolv_target *res2;
#endif
  ...
  for (res = daemon->resolv_files; res; res = res->next)
    {
      char *d, *path = safe_malloc(strlen(res->name) + 1);
#ifndef DIGI_RESOLV_SYMLINK_EXTENSION
      char *new_path;
      ...
      if ((d = strrchr(path, '/')))
	{
	  *d = 0; /* make path just directory */
	  res->wd = inotify_add_watch(daemon->inotifyfd, path, IN_CLOSE_WRITE | IN_MOVED_TO | IN_CREATE);
...
#ifdef DIGI_RESOLV_SYMLINK_EXTENSION
   for (res2 = daemon->resolv_files_target; res2; res2 = res2->next)
    {
      char *d, *path = safe_malloc(strlen(res2->name) + 1);

      strcpy(path, res2->name);
	...
	{
	  *d = 0; /* make path just directory */
	  res2->wd = inotify_add_watch(daemon->inotifyfd, path, IN_CLOSE_WRITE | IN_MOVED_TO);

	  res2->file = d+1; /* pointer to filename */
	  *d = '/';

	  if (res2->wd == -1 && errno == ENOENT)
	    die(_("directory %s for resolv-file-target is missing, cannot poll"), res2->name, EC_MISC);
	}

      if (res2->wd == -1)
	die(_("failed to create inotify for %s: %s"), res2->name, EC_MISC);
...
#ifdef DIGI_RESOLV_SYMLINK_EXTENSION
	  for (res2 = daemon->resolv_files_target; res2; res2 = res2->next)
	    if (res2->wd == in->wd && in->len != 0 && strcmp(res2->file, in->name) == 0)
	      hit = 1;
#endif
