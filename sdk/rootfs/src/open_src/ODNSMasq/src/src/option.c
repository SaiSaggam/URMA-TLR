...
#if defined(DIGI_RESOLV_SYMLINK_EXTENSION) && defined(HAVE_INOTIFY)
#define LOPT_RESOLV_FILE_TARGET	400
#endif
...
static const struct myoption opts[] = 
  ...
  { 
    ...
    { "resolv-file", 2, 0, 'r' },
#if defined(DIGI_RESOLV_SYMLINK_EXTENSION) && defined(HAVE_INOTIFY)
    { "resolv-file-target", 2, 0, LOPT_RESOLV_FILE_TARGET},
#endif
...
#if defined(DIGI_RESOLV_SYMLINK_EXTENSION) && defined(HAVE_INOTIFY)
  { LOPT_RESOLV_FILE_TARGET, ARG_DUP, "<path>", gettext_noop("Specify path to resolv.conf symlink target."), NULL },
#endif
...
#if defined(DIGI_RESOLV_SYMLINK_EXTENSION) && defined(HAVE_INOTIFY)
    case LOPT_RESOLV_FILE_TARGET: /* --resolv-file-target */
      {
	char *name = opt_string_alloc(arg);
	struct resolv_target *new, *list = daemon->resolv_files_target;

	if (name)
	  {
	    new = opt_malloc(sizeof(struct resolv_target));
	    new->next = list;
	    new->name = name;
	    list = new;
	  }
	daemon->resolv_files_target = list;
	break;
      }
#endif
