...
def send_rci(request):
    ...
    # Send device request
    response = self.session.post('http://%s/ws/sci' % self.hostname, data=req_data).content
...
def getNodeText(nodelist):
    ...
    for node in nodelist:
        if node.nodeType == node.TEXT_NODE:
            rc = rc + node.data
    return rc
class TimingTestCase(ic_testcase.TestCase):

    def test_timing_with_ds(self):
    
        """ Sends device request. 
        """
        self.log.info("**** Timing Test:  Device request")
        self.valid_target("ds_timing_test", 2048)
        self.valid_target("put_ds_timing_test", 100)

    def test_timing_with_rci_query_setting(self):
    
        """ Sends query_setting. 
        """
        self.log.info("**** Timing Test:  RCI query_setting")
    ...
    def test_timing_with_z_fw(self):
        ...
        self.log.info("**** Timing Test: Firmware Service")
