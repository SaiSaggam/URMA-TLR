...
def send_request(rci_request):
    ...
    try:
        expected_error = int(rci_request.error)
        rci_error_code = int(rci_request.error)
        response_error_code = int(error[0])

        if rci_error_code >= int(SERVER_ERROR_CODE) and response_error_code >= int(SERVER_ERROR_CODE):
            log.info('Error from server: %d' % rci_error_code)  
        elif response_error_code != rci_error:    
...
def test_query_setting_and_state():
    tests = [
            TestCase('<query_setting><unkownGroup/></query_setting>','2021','query_setting_unknown_group'),
            TestCase('<query_state><unkownGroup/></query_state>','2021','query_state_unknown_group'),
            ...
            TestCase('<query_setting><serial index="10"></serial></query_setting>',"2021",'query_setting_invalid_index_group'),
            TestCase('<query_state><device_state index="10"></device_state></query_state>',"2021",'query_state_invalid_index_group'),
            ...
            TestCase('<query_setting source="current" compare_to="defaults"></query_setting>',None,''),
            ]
...
def test_set_setting_and_state():
    tests = [
            TestCase('<set_setting><unkownGroup><someElement>12345</someElement></unkownGroup></set_setting>','2021','set_setting_unknown_group'),
            TestCase('<set_state><unkownGroup><someElement>12345</someElement></unkownGroup></set_state>','2021','set_state_unknown_group'),
            ...
            TestCase('<set_setting><system index="10"><description>Testing Invalid Index</description></system></set_setting>',"2021",'set_setting_invalid_index_group'),
            TestCase('<set_state><device_state index="10"><signed_integer>45</signed_integer></device_state></set_state>',"2021",'set_state_invalid_index_group'),
            ...
            TestCase('<set_setting><system/></set_setting>','2017','set_setting_no_element_1'),
            TestCase('<set_state><device_state/></set_state>','2017','set_state_no_element_1'),
            TestCase('<set_setting><system></system></set_setting>','2017','set_setting_no_element_2'),
            TestCase('<set_state><device_state></device_state></set_state>','2017','set_state_no_element_2'),
            TestCase('<set_setting><system><unknownElement>Testing Unkown Element</unknownElement></system></set_setting>','2007','set_setting_unknown_element'),
            TestCase('<set_state><device_state><unknownElement/></device_state></set_state>','2007','set_state_unknown_element'),
            TestCase('<set_setting><system><description>asdfghjklaasdfghjklaasdfghjklaasdfghjklaasdfghjklaasdfghjklaasdfghjkla</description></system></set_setting>','2021','set_setting_out_of_range'),
            TestCase('<set_state><device_state><signed_integer>9999999999999999999999999999999999999999999999999999999999999999999999999999999</signed_integer></device_state></set_state>','2021','set_state_very_out_of_range_above'),
            TestCase('<set_state><device_state><signed_integer>-9999999999999999999999999999999999999999999999999999999999999999999999999999999</signed_integer></device_state></set_state>','2021','set_state_very_out_of_range_below'),
            TestCase('<set_state><device_state><signed_integer>101</signed_integer></device_state></set_state>','2021','set_state_out_of_range_above'),
            TestCase('<set_state><device_state><signed_integer>-101</signed_integer></device_state></set_state>','2021','set_state_out_of_range_below'),
            TestCase("""<set_setting><system><description>Testing No Index
                Multiple lines
                to be bad test.</description></system></set_setting>""",'2021','set_setting_bad_type'),
            TestCase('<set_state><device_state><signed_integer>Some string value</signed_integer></device_state></set_state>','2021','set_state_bad_type'),
            TestCase('<set_state><gps_stats><latitude>49.0</latitude></gps_stats></set_state>','2021','set_read_only_value'),
            ]
...
def rci_write_only_validation(rci_request):
    request = RCI_BASE_TEMPLATE.substitute(cache=rci_request.cache,
                                            device_id=DEFAULT_DEVICE_ID,
                                            request=rci_request.rci,
                                            rci_attributes=rci_request.rci_attributes)
    ...
    try:
        expected_error = int(rci_request.error)
        if int(error[0]) != int(rci_request.error):    
            assert_equal(perr,rci_request.error, 'Expected: "%s" Recieved: "%s"' % (rci_request.error,perr))
    except:
         assert_equal(perr,rci_request.error, 'Expected: "%s" Recieved: "%s"' % (rci_request.error,perr))

    password = xpath.find('//password',doc)

    if len(password) > 0:
        childs = password[0].childNodes
        perr=childs[0].nodeValue
    else:
       perr=None

    assert_equal(perr,None, 'Expected: no response on "password" write only field but recieved: "%s"' % perr)


def test_query_write_only():

    tests = [
            TestCase('<query_setting><devicesecurity/></query_setting>', None,'query_password_write_only'),
            ]


    for test in tests:
        request = RCIRequest(test.test,test.error,test.name)
        yield rci_write_only_validation, request
