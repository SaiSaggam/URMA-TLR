...
class SendFileGetTestCase(ic_testcase.TestCase):
    ...
    def test_file_get(self):
        ...
        try:
            ...
            # Send device request
            file_get_response = self.session.post('http://%s/ws/sci' % self.hostname, data=my_file_getput_request)
            self.log.info("%s" % file_get_response.content)
            ...
            file_ls_response = self.session.post('http://%s/ws/sci' % self.hostname, data=my_file_ls_request)
            self.log.info("%s" % file_ls_response.content)
        finally:
