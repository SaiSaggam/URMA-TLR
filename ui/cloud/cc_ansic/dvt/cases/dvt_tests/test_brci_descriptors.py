...
class RciElement(object):
    ...
    def __init__(self, group, element):
        ...
        base = 16 if self.type in ['hex32', '0x_hex32'] else 10
...
class TestRciDescriptors(object):
    ...
    def set_and_query(self, test, setting=True):
        ...
        for element in xpath.find('//%s'%test.element.group,doc):
            ...
            if (test.element.access == 'read_only'):
                log.info("Checking Set on read-only element.")
                check_error = True
            elif test.error is None: 
                check_error = False         

            if not check_error: # Ensure we have no errors if we don't expect any.
                ...
                assert_not_equal(None, error,
                    "No errors were returned when something was expected %s." % test.error)

                if int(error[0]) != 2021:
                    assert_true(int(error[0]) in test.error,
                        "Unexpected error: %s : %s" % (error,test.error))
                else:
                    log.info("Server returns error.\n");
            ...
            if (test.error is None and test.element.access != 'read_only'):
                ...
                if test.element.type in ['hex32','0x_hex32']:
                    eq_('%s'%test.value.upper(),'%s'%new_value.upper(),
    ...
    def parse_group(self, descriptor, setting=True):
        ...
        for element in elements:
            ...
            elif element.type in ['hex32','0x_hex32']:
                if element.has_min():
                    hMin = hex(element.min)
                    hMinM = hex(element.min-1)
                    if 'hex32' == element.type:
