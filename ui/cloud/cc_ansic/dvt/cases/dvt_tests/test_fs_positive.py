...
class FileSystemTestCase(ic_testcase.TestCase):
    ...
    def test_fs4_ls(self):
        ...
        for hashAlgo in HashAlgoList:
            ...
            self.assertTrue(found_public, "Expected to find public directory.")
            self.assertTrue(found_private, "Expected to find private directory.")
    ...
    def put_a_file(self, fpath, fdata, fsize, offset = 0, truncate = False):
        ...
        response = self.session.post('http://%s/ws/sci' % self.hostname, data=putRequest).content
