...
from ..utils import get_filedatahistory, check_filedatahistory
...
def verify_the_file_on_server (instance, test_num, file_path, file_expected):
   ...
   if file_expected:
      ...
      if test_num == ds_archive:
         ...
         fdh = get_filedatahistory(instance.rest_session, file_path)
...
def send_device_put_request (instance, target, file_name):
   ...
   my_target_device_request = \
       ...
       </sci_request>""" % (instance.device_id, target, file_name)

   instance.log.info("Sending request to %s to get %s" %(target, file_name))

   # Send device request
   device_request_response = instance.session.post('http://%s/ws/sci' % instance.hostname, 
   ...
   instance.assertEqual(target_name, target, "returned target [%s] is not [%s]. response [%s]" %(target_name, target, device_request_response))
...
class PutRequestDvtTestCase(ic_testcase.TestCase):
...
#        self.process_put_request(ds_both, default_timeout, False)
    ...
    def process_put_request (self, test_num, timeout, file_expected):
        ...
        file_location = file_location + self.device_id + '/' + file_name
