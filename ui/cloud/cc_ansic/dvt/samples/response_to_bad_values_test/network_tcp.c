...
/* Note for maintinance: This file should be *almost* the same as cc_ansic/public/run/platforms/linux/network_tcp.c,
 * the only difference is that this writes to a file in app_network_tcp_close(). */
...
static connector_callback_status_t app_network_tcp_close(connector_network_close_t * const data)
{
    ...
    *fd = -1;
    ...
    if (data->status == connector_close_status_device_error)
    {
        snprintf(python_file_buffer, sizeof(python_file_buffer), "network_tcp_close connector_close_status_device_error\n");
