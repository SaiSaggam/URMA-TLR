...
#if (defined CONNECTOR_COMPRESSION_BUILTIN)
#include "zlib.h"
#endif
...
typedef connector_callback_status_t connector_msg_callback_t(connector_data_t * const connector_ptr, msg_status_t const msg_status, msg_session_t * const session, uint8_t const * buffer, size_t const length);
...
static void msg_send_complete(connector_data_t * const connector_ptr, uint8_t const * const packet, connector_status_t const status, void * const user_data);

static msg_session_t * msg_find_session(connector_msg_data_t const * const msg_ptr, uint16_t const id, msg_type_t const type)
{
...
static uint16_t msg_find_next_available_id(connector_msg_data_t * const msg_ptr)
{
...
static msg_session_t * msg_create_session(connector_data_t * const connector_ptr, uint16_t const service_id, msg_type_t const type)
{
    ...
    {
        void *ptr;
        connector_callback_status_t const status = malloc_data(connector_ptr, sizeof(msg_session_t), &ptr);
...
static void msg_delete_session(connector_data_t * const connector_ptr,  msg_session_t * const session, msg_type_t const type)
{
    connector_msg_data_t * const msg_ptr = get_facility_data(connector_ptr, E_MSG_FAC_MSG_NUM);
    ...
    free_data(connector_ptr, session);
...
static uint16_t msg_get_service_count(connector_msg_data_t const * const msg_ptr)
{
    uint16_t count = 0;
    uint16_t service_id;
...
static connector_callback_status_t msg_send_capabilities(connector_data_t * const connector_ptr, connector_msg_data_t const * const msg_data, uint8_t const flag)
{
    connector_callback_status_t status = connector_callback_busy;
    uint8_t * capabilty_packet = NULL;
    uint8_t * packet = get_packet_buffer(connector_ptr, E_MSG_FAC_MSG_NUM, &capabilty_packet);
    ...
    status = initiate_send_facility_packet(connector_ptr, packet, packet_len, E_MSG_FAC_MSG_NUM, release_packet_buffer, NULL);
...
static connector_callback_status_t msg_send_packet(msg_session_t * const session)
{
...
static connector_callback_status_t msg_send_data(connector_data_t * const connector_ptr, msg_session_t * const session, msg_data_info_t const * const info)
{
    connector_callback_status_t status = connector_callback_busy;
    connector_msg_data_t * const msg_ptr = get_facility_data(connector_ptr, E_MSG_FAC_MSG_NUM);
    ...
    if (info != NULL) 
    {
        ...
        session->bytes_in_frame = 0;
        session->last_chunk = (info->flag & CONNECTOR_DATA_REQUEST_LAST) != 0;
        #if (defined CONNECTOR_COMPRESSION_BUILTIN)
        session->compression = (info->flag & CONNECTOR_DATA_REQUEST_COMPRESSED) ? msg_ptr->peer_compression : MSG_NO_COMPRESSION;
        #endif
    }
    ...
    status = (session ->bytes_in_frame > 0) ? msg_send_packet(session) : connector_callback_continue;
...
static void * msg_send_start(connector_data_t * const connector_ptr, uint16_t const service_id, msg_data_info_t const * const info)
{
    msg_session_t * const session = msg_create_session(connector_ptr, service_id, msg_type_tx);

    ASSERT_GOTO(session != NULL, error);

    {
        connector_callback_status_t const status = msg_send_data(connector_ptr, session, info);
...
static void msg_send_complete(connector_data_t * const connector_ptr, uint8_t const * const packet, connector_status_t const status, void * const user_data)
{
    ...
    if (status == connector_success)
    {
        if (session->more_data)
        {
            if (!session->flow_controlled) 
                msg_send_data(connector_ptr, session, NULL);
    ...
    {
        connector_msg_callback_t * cb_fn = msg_ptr->service_cb[session->service_id];
        size_t const bytes = session->total_bytes - session->bytes_to_send;

        ASSERT_GOTO(cb_fn != NULL, done);
        cb_fn(connector_ptr, msg_status_send_complete, session, (session->user_data - bytes), bytes);
        if ((status != connector_success) || (!session->more_data && session->last_chunk))
            msg_delete_session(connector_ptr, session, msg_type_tx);
    }
...
static connector_callback_status_t msg_process_capabilities(connector_data_t * const connector_ptr, connector_msg_data_t * const msg_fac, uint8_t * const ptr)
{
    ...
    {
        status = msg_send_capabilities(connector_ptr, msg_fac, MSG_CAPABILITIES_RESPONSE);
    }
...
#if (defined CONNECTOR_COMPRESSION_BUILTIN)
static connector_callback_status_t msg_recv_compressed(connector_data_t * const connector_ptr, connector_msg_data_t const * const msg_fac, msg_session_t * const session, uint8_t * ptr, uint16_t length)
{
    connector_callback_status_t status = connector_callback_abort;
    z_streamp zlib_ptr = &session->zlib;
    ...
    do
    {
        ...
        if ((ret == Z_STREAM_END) || (zlib_ptr->avail_out == 0))
        {
            ...
            connector_msg_callback_t * cb_fn = msg_fac->service_cb[session->service_id];
            ...
            status = cb_fn(connector_ptr, state, session, session->frame, uncompressed_length);

            if (state == msg_status_end)
                msg_delete_session(connector_ptr, session, msg_type_rx);
        }
...
static connector_callback_status_t msg_process_data(connector_data_t * const connector_ptr, connector_msg_data_t const * const msg_fac, uint8_t * ptr, uint16_t const length, bool const start)
{
    connector_callback_status_t status = connector_callback_abort;
    ...
    if (start)
    {
        ...
        session = msg_create_session(connector_ptr, service_id, msg_type_rx);
        ASSERT_GOTO(session != NULL, error);
        session->session_id = session_id;
    ...
    session->last_chunk = (flag & MSG_FLAG_LAST_DATA) == MSG_FLAG_LAST_DATA;
    {
        ...
        #if (defined CONNECTOR_COMPRESSION_BUILTIN)
        if (session->compression == MSG_COMPRESSION_LIBZ) 
            status = msg_recv_compressed(connector_ptr, msg_fac, session, data_ptr, data_length);
        else
        #endif
        {
            connector_msg_callback_t * cb_fn = msg_fac->service_cb[session->service_id];
            ...
            if (session->last_chunk)
                state = msg_status_end;
            status = cb_fn(connector_ptr, state, session, data_ptr, data_length);
            if (session->last_chunk) 
                msg_delete_session(connector_ptr, session, msg_type_rx);
        }
    }
...
static connector_callback_status_t msg_process_ack(connector_data_t * const connector_ptr, connector_msg_data_t const * const msg_fac, uint8_t const * ptr)
{
    connector_callback_status_t status = connector_callback_continue;
    msg_session_t * session;
    ...
    if (session->available_window > 0)
    {
        if (session->flow_controlled) 
        {
            ...
            if (session->more_data) 
                status = msg_send_data(connector_ptr, session, NULL);
        }
    }
...
static connector_callback_status_t msg_process_error(connector_data_t * const connector_ptr, connector_msg_data_t const * const msg_fac, uint8_t * const ptr)
{
    connector_callback_status_t status = connector_callback_abort;
   ...
   
    {
...

        {
            ...
            status = cb_fn(connector_ptr, msg_status_error, session, &error, sizeof error);
            msg_delete_session(connector_ptr, session, msg_type_tx);
        }
    }
...
static connector_callback_status_t msg_discovery(connector_data_t * const connector_ptr, void * const facility_data, uint8_t * const packet)
{
    UNUSED_PARAMETER(packet);

    return msg_send_capabilities(connector_ptr, facility_data, MSG_FLAG_REQUEST);
}

static connector_callback_status_t msg_process(connector_data_t * const connector_ptr, void * const facility_data, uint8_t * const edp_header)
{
    connector_callback_status_t status = connector_callback_continue;
    ...
    switch (opcode)
    {
        case msg_opcode_capability:
            status = msg_process_capabilities(connector_ptr, msg_ptr, ptr);
            break;

        case msg_opcode_start:
            status = msg_process_data(connector_ptr, msg_ptr, ptr, length, true);
            break;

        case msg_opcode_data:
            status = msg_process_data(connector_ptr, msg_ptr, ptr, length, false);
            break;

        case msg_opcode_ack:
            status = msg_process_ack(connector_ptr, msg_ptr, ptr);
            break;

        case msg_opcode_error:
            status = msg_process_error(connector_ptr, msg_ptr, ptr);
            break;
...
static connector_callback_status_t msg_process_pending(connector_data_t * const connector_ptr)
{
    connector_callback_status_t status = connector_callback_continue;
    connector_msg_data_t * const msg_ptr = get_facility_data(connector_ptr, E_MSG_FAC_MSG_NUM);
    ...
    if ((msg_ptr != NULL) && (msg_ptr->pending != NULL))
    {
        ...
        {
            status = initiate_send_facility_packet(connector_ptr, session->frame, session->bytes_in_frame, E_MSG_FAC_MSG_NUM, msg_send_complete, session);
            ...
            msg_send_complete(connector_ptr, NULL, connector_success, session);
    }

    return status;
}

static void msg_delete_all_sessions(connector_data_t * const connector_ptr,  connector_msg_data_t * const msg_ptr, uint16_t const service_id)
{
    msg_type_t msg_type;
    ...
    for (msg_type = msg_type_tx; msg_type < msg_type_count; msg_type++) 
    {
        ...
        while(session != NULL)
        {
            ...
            if (session->service_id ==service_id )
                msg_delete_session(connector_ptr, session, msg_type);
            session = next_session;
        }
    }
}

static connector_callback_status_t msg_delete_facility(connector_data_t * const connector_ptr, uint16_t const service_id)
{
    connector_callback_status_t status = connector_callback_abort;
    connector_msg_data_t * const msg_ptr = get_facility_data(connector_ptr, E_MSG_FAC_MSG_NUM);
    ...
    /* is list empty? */
    {
        ...
        msg_delete_all_sessions(connector_ptr, msg_ptr, service_id);
        status = is_empty ? del_facility_data(connector_ptr, E_MSG_FAC_MSG_NUM) : connector_callback_continue;
    }
...
static uint16_t msg_init_facility(connector_data_t * const connector_ptr, uint16_t service_id, connector_msg_callback_t callback)
{
    connector_callback_status_t status = connector_callback_abort;
    ...
    if (msg_ptr == NULL)
    {
        ...
        status = add_facility_data(connector_ptr, E_MSG_FAC_MSG_NUM, &fac_ptr, sizeof *msg_ptr, msg_discovery, msg_process);
