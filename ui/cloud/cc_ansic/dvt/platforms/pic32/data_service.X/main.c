...
#if defined(__PIC32MX__) && defined(CONNECTOR_DATA_SERVICE)
...
int main(void){
    ...
    // Initialize iDigi variables into connector_config.
    InitIDigiConfig();
    ...
    while(1){
        ...
        if(cur_addr.Val != AppConfig.MyIPAddr.Val){
            ...
            if(ip_assigned){
                if(connector_config.socket_fd != -1 && handle != NULL){
        ...
        if(ip_assigned){
            ...
            if(handle == NULL || status == connector_device_terminated){
                if(status == connector_device_terminated){
                    free(handle);
                }
            ...
            if(!connector_config.connected && connector_config.socket_fd != -1){
                ...
                if(connect_time == 0){
                    ...
                    connector_config.connected = true;
                    # if defined(CONNECTOR_DATA_SERVICE)
                    {
                        connector_data_service_put_request_t data_service_request;
                        data_service_request.content_type = content_type;
                        data_service_request.path = path;
                        char buffer[50];
                        int time = SNTPGetUTCSeconds();
                        sprintf(buffer, connect_log_proto, time, connector_config.server_url);
            ...
            #if defined(__PIC32MX__) && defined(CONNECTOR_DATA_SERVICE)
            {
                int switch_on;
                int i=0;
                while(connector_config.connected && i < 2){
...
static int get_uptime(void){
    ...
    uint32_t up_time = (cur_time - connector_config.start_time);
    return up_time;
}
...
static void InitIDigiConfig(void){
    ...
    // deterministic for checksum generation.
    memset((void*)&connector_config, 0x00, sizeof(connector_config));
    connector_config.start_time = TickGet() / TICKS_PER_SECOND;
    // TODO: Derive Device Type.
    connector_config.device_type = "PIC32 Ethernet Starter Kit";
    connector_config.rx_keepalive = 30;
    connector_config.tx_keepalive = 30;
    connector_config.wait_count = 3;
    connector_config.vendor_id[0] = 1;
    connector_config.vendor_id[1] = 0;
    connector_config.vendor_id[2] = 0;
    connector_config.vendor_id[3] = 5;
    connector_config.server_url = "test.idigi.com";
    connector_config.connection_type = connector_lan_connection_type;
...
connector_callback_status_t connector_callback(connector_class_id_t const class_id,
    ...
    switch(class_id){
        ...
        case connector_class_operating_system:
            status = connector_os_callback(request_id.os_request,
