...
def check_release_build():
    ...
    config_file_name = "./connector_config.h"

    isDebug = False
    isCompression = False
    # If connector_config doesn't exist (1.1 case) then just assume debug 
    # and compression not enabled.
    if os.path.isfile(config_file_name):
        infile = open(config_file_name, "r")
        text = infile.read()
        infile.close()
        if text.find('CONNECTOR_NO_DEBUG') == -1:
            isDebug = True
        if text.find('CONNECTOR_NO_COMPRESSION') == -1:
            isCompression = True
...
# --------------------------------------------------------------
    ...
    if found_section_rodata_str1_1 == 1:     
        ...
        if len(elements) == 3:
            ...
            if object_file.endswith('connector_api.o'):
...
# --------------------------------------------------------------
    ...
    if len(elements) == 4:
        ...
        if elements[0] =='.text' and object_file.endswith('connector_api.o'):
        ...
        if elements[0] =='.rodata' and object_file.endswith('connector_api.o'):
