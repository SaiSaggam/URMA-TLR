...
## Download setuptools installation script from https://bootstrap.pypa.io/ez_setup.py
## install it: sudo python ez_setup.py
## Launch buildout again
###############################################################################
# increaseStepNumber
# echo ""
# echo ""
...
#### CREATE A CACHE SYSTEM TO BUILD PYTHON
###############################################################################
increaseStepNumber
echo ""
echo ""
echo "### Step $STEPCOUNTER: Create share folder to store the download cache system for Python build system."
DIRECTORY=/tmp/.buildout_cache
if [ -d "$DIRECTORY" ]; then
    # Control will enter here if $DIRECTORY exists.
    echo ""
    echo "Buildout cache system folder exists, it is not necessary create it."
else
    echo "Create the Buildout cache system folder in "$DIRECTORY
    mkdir $DIRECTORY
fi
