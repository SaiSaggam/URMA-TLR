...
class DiscoveryTestCase(cc_testcase.TestCase):
    ...
    def test_verify_device_type_matches(self):
        """ This test intends to test if the application was able to connect reporting a previously autoprovisioned Device ID
        to Device Cloud and is connected. Also verifies that the Device Type reported by Device Cloud is the same than the one 
        configured in Cloud Connector. """
    ...
    def test_verify_vendor_id_matches(self):
        """ This test intends to test if the application was able to connect reporting a previously autoprovisioned Device ID
        to Device Cloud and is connected. Also verifies that the Vendor ID reported by Device Cloud is the same than the one 
        configured in Cloud Connector. """
