...
class DeviceRequestDvtTestCase(cc_testcase.TestCase):
    ...
    def test_01_send_busy_request_target(self):
        """ Verifies that the target "busy request" returns the correct response in base64 format.
        ...
        if(status):
            ...
            if(responseText == encodedPayload):
                ...
                result, line, dataBuffer = self.deviceHandler.readUntilPattern ( pattern="Total Payload received = ", timeout=30)
                if(result):
    ...
    def test_02_send_cancel_request_target(self):
        """ Verifies that the target "cancel request" returns the correct response in base64 format.
    ...
    def test_03_send_cancel_in_the_middle_target(self):
        """ Verifies that the target "cancel in the middle" returns the correct response in base64 format.
        Connector must manage correctly the payload received but abort the response and send error.
        """
    ...
    def test_04_send_cancel_response_target(self):
        """ Verifies that the target "cancel response" returns the correct response in base64 format.
        Connector must manage correctly the payload received and start writing the response, but abort it before the completion
        """
    ...
    def test_05_send_busy_response_target(self):
        """ Verifies that the target "busy response" returns the correct response in base64 format.
    ...
    def test_06_send_timeout_response_target(self):
        """ Verifies if the connector behaves correctly with the target "timeout response" in base64 format.
