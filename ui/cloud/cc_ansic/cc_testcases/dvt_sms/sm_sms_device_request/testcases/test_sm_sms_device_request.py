...
class DeviceRequestSmsTestCase(cc_testcase.SMSTestCase):
       
    def test_sm_user_msg_sms(self):
        """ User msg with reply for expected target """
        received_data = self.send_sm_sms_request("User request", '<user_msg path="myTarget">My device request data</user_msg>', True)
        self.assertEqual("My device response data", received_data, "received_data do not match what is expected")

    def test_sm_user_msg_noresp_sms(self):
        """ User msg with no reply """
        self.send_sm_sms_request("user_msg no reply", '<user_msg path="myTarget">My device request data</user_msg>', False)

    def test_sm_user_msg_nulltarget_sms(self):
        """ User msg with reply for empty target """
        received_data = self.send_sm_sms_request("user_msg empty target", '<user_msg>My device request data</user_msg>', True)
        self.assertEqual("Empty target in SM device request", received_data, "received_data do not match what is expected")

    def test_sm_user_msg_badtarget_sms(self):
        """ User msg with reply for unexpected target """
        received_data = self.send_sm_sms_request("user_msg bad target", '<user_msg path="badTarget">My device request data</user_msg>', True)
    ...
    def send_sm_sms_request (self, test_name, command, reply_needed):
        ...
        if reply_needed == True:
            ...
            ## Check if exist entry 'sms'
            #if( not element.has_key("sms") ):
                #self.fail("Response has not element sms")
