...
/* If not defined, app will cancel just the desired action */
/* #define USE_CANCEL_ALL */

#ifndef USE_CANCEL_ALL
static uint32_t request_id[connector_transport_all];	/* Only used for UDP and SMS transports */
#endif
    ...
    do
    {
        ...
        /* update location */
        {
            ...
            switch(location_case)
            {
                ...
                {
                    static char latitude_str[] = "42.34";
                    static char longitude_str[] = "71.20";
                    static char elevation_str[] = "62.10";
...
{
    size_t const path_max_size = sizeof("_dpn_tcp_double_");
    static char * app_transport[] = {"tcp", 
#if (defined CONNECTOR_TRANSPORT_UDP)
                                     "udp",
#endif
                                     "sms"};
    ...
    snprintf(path_name, path_max_size, "dpn_%s_%s", app_transport[transport], app_type[type]);
...
{
    connector_request_data_point_t * dp_ptr = malloc(sizeof(connector_request_data_point_t));
    ...
    {
        static size_t response_needed = 0;

        /* response_required for 1/3 of the messages. If we set it to 1/2, then only udp
           are sent with response_required */
        dp_ptr->response_required = (response_needed % 3)?connector_false:connector_true;
        response_needed++;
        ...
        if(dp_ptr->response_required)
        {
            dp_ptr->request_id = &request_id[dvt_ptr->transport];
        }
        else
#endif
        {
...
{
    size_t const rand_bytes = (rand() % 512) + 1;
...
connector_status_t app_send_data_point(connector_handle_t const handle)
{
    ...
    connector_bool_t response_required = connector_false;
    ...
    switch (status)
    {
        case connector_success:
            if (dvt_ptr->transport != connector_transport_tcp)
            {
                if (dvt_ptr->data_type == connector_data_point_type_binary)
                {
                    response_required = ((connector_request_data_point_binary_t *)(dvt_ptr->dp_ptr))->response_required;
                }
                else
                {
                    response_required = ((connector_request_data_point_t *)(dvt_ptr->dp_ptr))->response_required;
                }
            }
            ...
            if (response_required)
            {
                ...
                cancel_request.transport = dvt_ptr->transport;
                
#ifdef USE_CANCEL_ALL
                cancel_request.request_id = 0;

                APP_DEBUG("Trying to cancel all pending %s sessions\n", (cancel_request.transport == connector_transport_sms) ? "SMS" : "UDP");
#else
                cancel_request.request_id = request_id[cancel_request.transport];

                APP_DEBUG("Trying to cancel pending %s session with request_id=%d\n", (cancel_request.transport == connector_transport_sms) ? "SMS" : "UDP", cancel_request.request_id);
#endif
                do
                {
#ifdef USE_CANCEL_ALL
...
connector_callback_status_t app_data_point_handler(connector_request_id_data_point_t const request_id, void * const data)
{
    ...
    switch (request_id)
    {
        case connector_request_id_data_point_binary_response:
        {
            ...
            if (first_time)
            {
                APP_DEBUG("Returning busy for %s response\n", dp_ptr->path);
        ...
        case connector_request_id_data_point_binary_status:
        {
            ...
            if (first_time)
            {
                APP_DEBUG("Returning busy for %s status\n", dp_ptr->path);
                first_time = connector_false;
            }
