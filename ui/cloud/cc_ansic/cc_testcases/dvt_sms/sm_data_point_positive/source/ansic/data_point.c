...
static connector_data_point_t * get_stream_points(connector_data_point_type_t const type)
{
    ...
    APP_DEBUG("get_stream_points: num_of_points=%" PRIsize "\n",num_of_points);
    ...
    do
    {
        switch (type)
        {
            ...
            case connector_data_point_type_string:
            {
                ...
                APP_DEBUG("connector_data_point_type_string [%s]\n", value[test_case % AV_STR_PATTERNS]);
                break;
            }
...
static void * get_data_point(dvt_dp_t * dvt_ptr)
{
    ...
    if (dp_ptr->transport != connector_transport_tcp)
    {
        ...
        if (dp_ptr->response_required)
            dp_ptr->timeout_in_seconds = (dp_ptr->transport == connector_transport_udp)?CONNECTOR_SM_UDP_TX_TIMEOUT:CONNECTOR_SM_SMS_TX_TIMEOUT;
        response_needed++;
    }
...
static void * get_binary_point(size_t * bytes, connector_transport_t transport)
{
    size_t rand_bytes;
    unsigned char * data_ptr;

    if (transport == connector_transport_sms)
    {
        /* For SMS, limit to a max of 6 (768/128) messages */
        rand_bytes = (rand() % 768) + 1;
    }
    else
    {
        rand_bytes = (rand() % 16384) + 1;
    }

    data_ptr = malloc(rand_bytes);
...
static void * get_data_point_binary(dvt_dp_t * dvt_ptr)
{
    ...
    dp_ptr->path = NULL;
    dp_ptr->transport = dvt_ptr->transport;
    dp_ptr->point = get_binary_point(&dp_ptr->bytes_used, dvt_ptr->transport);
