...
"""<sci_request version="1.0">
    <send_message synchronous="true" syncTimeout="900">
        <targets>
            <device id="%s"/>
        </targets>
        <sms>
...
class SmsConnectTestCase(cc_testcase.SMSTestCase):
    ...
    def test_update_1_firmware(self):
        ...
        if ( not result ):
            self.failAndStopAllTests("Response to update_firmware was not sucessfully")
        ...
        if ( not result ):
            self.failAndStopAllTests("Console feedback for update_firmware completed NOT arrived")
    ...
    def send_sm_disconnect_request(self):
        ...
        if ( not result ):
            self.failAndStopAllTests("Response to disconnect was not sucessfully")
