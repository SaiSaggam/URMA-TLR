...
static connector_status_t start_transport(connector_handle_t handle)
{
    ...
    {
        APP_DEBUG("Failed to start the SMS transport layer. Error: %d\n", status);
    }
...
{
    connector_initiate_stop_request_t const transport = {connector_transport_sms, connector_stop_immediately};
    connector_status_t status;
    ...
    if (status != connector_success)
    {
        APP_DEBUG("Failed to stop the SMS transport layer. Error: %d\n", status);
    }
...
int application_run(connector_handle_t handle)
{
    ...
    unsigned int const ping_interval_in_seconds = 40;
    ...
    while (1)
    {
        ...
        switch (status)
        {
            ...
            case connector_success:
                ...
                {
                    #if 0
                    /* As SMS does not support BatteryOperatedMode it's not safe to stop the transport.
                       The *.py dvt can have initiated a request that can be lost by the connector */
                    APP_DEBUG("stop/start transport\n");
