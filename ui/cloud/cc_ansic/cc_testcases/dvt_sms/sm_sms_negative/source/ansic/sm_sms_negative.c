...
connector_status_t app_send_ping(connector_handle_t handle)
{
    ...
    APP_DEBUG("Sent ping [%d] with request_id=%d.\n", status, *request.request_id);
    if (status == connector_success)
    {
        ...
        if (test_cases < 2)
        {
            ...
            APP_DEBUG("Trying to cancel the ping request with request_id=%d (response_required = %d)\n", cancel_request.request_id, request.response_required);
            do
            {
...
connector_status_t app_send_data(connector_handle_t handle)
{
    ...
    if (status == connector_success)
    {
        if (test_cases < 2)
        {
            ...
            APP_DEBUG("Trying to cancel the send data request with request_id=%d (response_required=%d)\n", cancel_request.request_id, header_ptr->response_required);
            do
            {
                status = connector_initiate_action(handle, connector_initiate_session_cancel, &cancel_request);
                if (status == connector_service_busy)
...
static connector_callback_status_t app_handle_put_request(connector_request_id_data_service_t const request_id, void * const cb_data)
{
    ...
    switch (request_id)
    {
        case connector_request_id_data_service_send_data:
        {
            ...
            APP_DEBUG("SM negative -> put request -> send_data\n");
            if (!(test_case % 2))
            {
        ...
        case connector_request_id_data_service_send_response:
        {
            ...
            APP_DEBUG("SM negative -> put request -> send_response\n");
            if (!(test_case % 2))
            {
        ...
        case connector_request_id_data_service_send_status:
        {
            ...
            switch (status_ptr->status)
            {
                case connector_data_service_status_complete:
                    APP_DEBUG("Put request status complete.\n");
                    break;

                case connector_data_service_status_cancel:
                    APP_DEBUG("Put request status cancel.\n");
                    break;

                default:
                    APP_DEBUG("Put request status error [%d].\n", status_ptr->status);
                    break;
            }
