...
typedef enum {
    ...
    dvt_error_put_busy,
    dvt_error_put_start,
    dvt_error_put_middle,
    dvt_error_put_end,
    dvt_error_put_timeout
} error_cases_t;

typedef struct {
    connector_file_offset_t offset;
    error_cases_t error_case;
} dvt_put_error_entry_t;

static dvt_put_error_entry_t dvt_put_error_list[] =
{
    {DVT_BUSY_OFFSET, dvt_error_put_busy},
    {DVT_START_OFFSET, dvt_error_put_start},
    {DVT_MIDDLE_OFFSET, dvt_error_put_middle},
    {DVT_END_OFFSET, dvt_error_put_end},
    {DVT_TIMEOUT_OFFSET, dvt_error_put_timeout}
};

static put_steps_t current_step; /* update_error_case() sets this to start */
static error_cases_t current_error_case;

static void update_error_case(connector_file_offset_t offset)
{
    size_t i;
    size_t const dvt_error_list_count = sizeof(dvt_put_error_list)/sizeof(dvt_put_error_list[0]);

    current_error_case = dvt_error_none;
    current_step = start;
    for (i = 0; i < dvt_error_list_count; i++)
    {
        if (dvt_put_error_list[i].offset == offset)
        {
            current_error_case = dvt_put_error_list[i].error_case;
